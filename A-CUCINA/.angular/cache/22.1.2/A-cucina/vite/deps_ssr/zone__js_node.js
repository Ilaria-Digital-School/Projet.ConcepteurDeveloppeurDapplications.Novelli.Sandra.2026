import { n as __require$1 } from "./rolldown-runtime-C7HZzL1F.js";
//#region node_modules/zone.js/fesm2015/zone-node.js
/**
* @license Angular
...
__symbol__(ADD_EVENT_LISTENER_STR);
__symbol__(REMOVE_EVENT_LISTENER_STR);
var TRUE_STR = "true";
var FALSE_STR = "false";
...
function patchMicroTask(obj, funcName, metaCreator) {
	...
	setNative = patchMethod(obj, funcName, (delegate) => function(self2, args) {
		const meta = metaCreator(self2, args);
		if (meta.cbIdx >= 0 && typeof args[meta.cbIdx] === "function") return Zone.current.scheduleMicroTask(meta.name, args[meta.cbIdx], meta, scheduleTask);
		else return delegate.apply(self2, args);
	});
}
...
function patchEvents(Zone2) {
	Zone2.__load_patch("EventEmitter", (global2, Zone3, api) => {
		...
		if (events && events.EventEmitter) patchEventEmitterMethods(events.EventEmitter.prototype);
	});
}
function patchFs(Zone2) {
	Zone2.__load_patch("fs", (global2, Zone3, api) => {
		...
		[
			...
			"writeFile",
			"writev"
		].filter((name) => !!fs[name] && typeof fs[name] === "function").forEach((name) => {
...
function patchNode(Zone2) {
	...
	Zone2.__load_patch("handleUnhandledPromiseRejection", (global2, Zone3, api) => {
		...
		function findProcessPromiseRejectionHandler(evtName) {
			return function(e) {
				findEventTasks(process, evtName).forEach((eventTask) => {
					if (evtName === "unhandledRejection") eventTask.invoke(e.rejection, e.promise);
					else if (evtName === "rejectionHandled") eventTask.invoke(e.promise);
				});
			};
		}
	});
	Zone2.__load_patch("crypto", () => {
		...
		if (crypto) ["randomBytes", "pbkdf2"].forEach((name) => {
	...
	Zone2.__load_patch("console", (global2, Zone3) => {
		[
			...
			if (originalMethod) console[m] = function() {
				const args = ArraySlice.call(arguments);
				if (Zone3.current === Zone3.root) return originalMethod.apply(this, args);
				else return Zone3.root.run(originalMethod, this, args);
			};
		});
	});
